...
**같은 세션을 유지하는 경우 (권장)**
- "마일스톤 N 시작해줘"라고만 하면 됩니다.
- Claude Code가 `docs/` 전체 내용과 이전 작업 맥락을 기억하므로 문서를 별도로 붙여넣을 필요가 없습니다.

**새 세션을 시작하는 경우**
- 아래 지시문을 참고해 관련 `docs/` 문서를 명시하고 작업 목록을 함께 전달하세요.
...
- 같은 세션을 유지하면 Claude Code가 `docs/` 전체 맥락, 이전 수정 내역, 오류 히스토리를 기억합니다.
- 세션이 끊어졌다면 "이전에 마일스톤 N까지 완료했어, 마일스톤 N+1 시작해줘"처럼 현재 위치만 알려주세요.
- Claude Code가 파일 생성·편집·`flutter analyze` 실행을 직접 수행하므로 복사·붙여넣기가 필요 없습니다.

**잘 동작하는 지시 패턴**

- 마일스톤 단위로 시작: "마일스톤 3 시작해줘"
- 오류 발생 시 그대로 전달: "이 오류 수정해줘" (오류 메시지 첨부)
- 특정 파일 기준 명시: "08_gps_algorithm.md의 GpsIndexer 그대로 구현해줘"
